"""
UltraSam regression training script with LoRA fine-tuning.
"""
...
class RegressionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for col in self.point_cols:
            xy = ast.literal_eval(row[col])  # [x, y]
...
class UltraSamRegressorLoRA(nn.Module):
    def __init__(
        ...
        # Apply LoRA to the attention projection layers
        ...
        self.backbone.print_trainable_parameters()  # log how many params are being trained
    ...
    def forward(self, x):
        ...
        if feat.ndim == 4:
            ...
            feat = feat.mean(dim=1)       # (B, seq, C) → (B, C)
...
def mean_radial_error(pred_norm, gt_norm, orig_wh):
    ...
    wh = orig_wh.unsqueeze(1).repeat(1, num_points, 1)
    pred_px = pred_norm.view(B, num_points, 2) * wh
    gt_px   = gt_norm.view(B, num_points, 2) * wh
    dists = (pred_px - gt_px).pow(2).sum(dim=-1).sqrt()
...
def main():
    ...
    parser.add_argument("--output_dir", default="reg_runs/lora")
    ...
    print(f"LoRA r={args.lora_r}, alpha={args.lora_alpha}, lr={args.lr}")
